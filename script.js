import { initializeApp } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-app.js";
        ...
        function getQuestionSolverHtml() {
            return `
                <div class="mb-4">
                    <nav class="flex space-x-1 caderno-tabs-container">
                ...
                <div class="bg-white rounded-b-lg rounded-tr-lg shadow-sm -mt-4">
                     <div class="p-4 border-b border-gray-200">
                        <h3 id="question-counter-top" class="flex items-baseline text-xl text-gray-800"></h3>
                    </div>
                    <div class="flex justify-between items-center p-4 border-b border-gray-200 text-sm">
                        <div id="question-info-container" class="text-gray-600 space-y-1"></div>
                        <div id="question-toolbar" class="flex items-center space-x-4 text-gray-600"></div>
                    </div>
                     <div class="p-6">
                        <div id="caderno-question-view">
                            <div id="caderno-questions-container" class="space-y-6"></div>
                            <div id="caderno-navigation-controls" class="flex justify-start items-center mt-8 space-x-4"></div>
                        </div>
                        <div id="caderno-stats-view" class="hidden">
                             <div id="caderno-stats-content"></div>
                        </div>
                     </div>
                </div>
            `;
        ...
        async function renderFoldersAndCadernos() {
             ...
             if (currentCadernoId) {
                ...
                // Injeta a UI de resolução de questões
                savedCadernosListContainer.innerHTML = vadeMecumContentArea.querySelector('#tabs-and-main-content').outerHTML;

                // Esconde/mostra os elementos corretos para o contexto do caderno
                const cq_container = savedCadernosListContainer.querySelector('#questions-container');
                const cs_view = savedCadernosListContainer.querySelector('#stats-view');
                ...
                savedCadernosListContainer.querySelector('#tabs-container').addEventListener('click', async (event) => {
                    const targetTab = event.target.dataset.tab;
                    if (targetTab) {
                        savedCadernosListContainer.querySelectorAll('.tab-button').forEach(btn => btn.classList.remove('active'));
                        event.target.classList.add('active');

                        if (targetTab === 'question') {
                           savedCadernosListContainer.querySelector('#question-view').classList.remove('hidden');
                           cs_view.classList.add('hidden');
                           ...
                           if (questionToolbar) questionToolbar.classList.remove('hidden');
                            ...
                            savedCadernosListContainer.querySelector('#question-view').classList.add('hidden');
                            cs_view.classList.remove('hidden');
                            ...
                            const statsContainer = cs_view.querySelector('#stats-content');
        ...
        window.addEventListener('click', function(e){   
          ...
          if (loadModal && !loadModal.querySelector('div').contains(e.target) && !savedFiltersListBtn.contains(e.target) && !loadModal.classList.contains('hidden')) {
              loadModal.classList.add('hidden');
          }
           if (saveModal && !saveModal.querySelector('div').contains(e.target) && !saveFilterBtn.contains(e.target) && !saveModal.classList.contains('hidden')) {
              saveModal.classList.add('hidden');
          }
          if (authModal && authModal.contains(e.target) && !authModal.querySelector('div').contains(e.target)) {
